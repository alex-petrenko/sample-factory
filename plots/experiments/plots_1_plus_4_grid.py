...
PLOTS = [
    ...
    dict(key='0_aux/avg_rewraw_pos', name='Avg. distance to the target', label='Avg. distance, meters',
         coeff=1.0 / EPISODE_DURATION),
    dict(key='0_aux/avg_num_collisions_after_settle', name='Avg. collisions between drones per minute',
         label='Number of collisions', logscale=True, coeff=COLLISIONS_SCALE),
    dict(key='0_aux/avg_rewraw_crash', name='Flight performance', label='% of the episode in the air',
         coeff=CRASH_GROUND_SCALE, mutate=lambda y: 100 - y),
]
PLOT_STEP = int(5e6)
