...
# Run locally: python -m sample_factory.runner.run --run=sf_examples.isaacgym_examples.experiments.isaacgym_ant --runner=processes --max_parallel=2 --experiments_per_gpu=2 --num_gpus=1
# Run on Slurm: python -m sample_factory.runner.run --run=sf_examples.isaacgym_examples.experiments.isaacgym_ant --runner=slurm --slurm_workdir=./slurm_isaacgym --experiment_suffix=slurm --slurm_gpus_per_job=1 --slurm_cpus_per_gpu=16 --slurm_sbatch_template=./sample_factory/runner/slurm/sbatch_timeout.sh --pause_between=1 --slurm_print_only=False
