...
class InferenceWorker(EventLoopObject, Configurable):
    def __init__(
        ...
        self.policy_outputs = buffer_mgr.policy_output_tensors
        ...
        with inference_context(self.cfg.serial_mode):
            with timing.add_time('deserialize'):
                ...
                for actor_idx, split_idx, request_data in self.requests:
                    for traj_slice, rollout_step in request_data:
                        idx = (traj_slice, rollout_step)
                        dict_of_lists_append(obs, self.obs_tensors, idx)
                        rnn_states.append(self.rnn_state_tensors[idx])
