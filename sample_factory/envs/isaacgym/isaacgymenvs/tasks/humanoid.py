...
from isaacgym.torch_utils import to_torch, get_axis_params, quat_conjugate, torch_rand_float, tensor_clamp, \
    normalize_angle, unscale

from sample_factory.envs.isaacgym.isaacgymenvs.tasks.base.vec_task import VecTask
from sample_factory.envs.isaacgym.isaacgymenvs.utils.torch_jit_utils import compute_heading_and_up, compute_rot

project_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.append(project_dir)


class HumanoidRun(VecTask):
    ...
    def __init__(self, cfg, sim_device, headless):
        ...
        self.true_objective = torch.zeros([self.num_envs], device=self.device)
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf, progress_reward = compute_humanoid_reward(
            self.obs_buf,
            self.reset_buf,
            self.progress_buf,
            self.actions,
        ...
        self.true_objective += progress_reward

        episode_cumulative = dict()
        episode_cumulative['progress_reward'] = progress_reward
...
def compute_humanoid_reward(
    ...
    return total_reward, reset, progress_reward
